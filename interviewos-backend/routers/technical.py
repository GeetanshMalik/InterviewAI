...
async def clear_answers(interview_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if not interview or interview["userId"] != current_user["id"]:
        raise HTTPException(status_code=404, detail="Interview not found")
    await asyncio.to_thread(repository_service.clear_round_state, interview_id, "technical", commit=False)
    repository_service.create_log(
        interview_id,
        "warning",
        "Technical Agent",
        "Technical round answers cleared for same-round restart.",
        "technical",
        commit=False,
    )
    await repository_service.commit_local_async()
    repository_service.commit_mirror_background()
    return {"message": "Technical answers cleared"}


@router.post("/answers")
async def submit_answer(payload: TechnicalAnswerRequest, current_user: dict = Depends(get_current_user)):
    interview = store.interviews.get(payload.interview_id)
    if not interview or interview["userId"] != current_user["id"]:
        raise HTTPException(status_code=404, detail="Interview not found")
    question = next((item for item in store.technical_questions.get(payload.interview_id, []) if item["id"] == payload.question_id), None)
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
    answer_source = payload.answer_source or "spoken_submit"
    answer_mode = str(payload.answer_mode or question.get("answer_mode") or "spoken").lower()
    if answer_mode not in {"spoken", "code"}:
        answer_mode = "spoken"
    ...
    if answer_mode == "code":
        ...
        speech_metrics = {
            "averageConfidence": 1 if payload.answer.strip() else 0,
            "wordsPerMinute": 0,
            "durationSeconds": payload.time_taken_seconds or 0,
            "longPauseCount": 0,
            "unclearCount": 0,
            "transcriptWords": len(payload.answer.split()),
            "confidenceLabel": "steady" if payload.answer.strip() else "unclear",
            "notes": ["Technical code question: scoring uses submitted code only, not speech detection."],
            "realtimeSignals": [],
        }
    existing_answer = next(
        (
            item
            for item in store.technical_answers.get(payload.interview_id, [])
    ...
    if existing_answer:
        ...
        cached_answer["runtime"] = build_round_runtime_state(interview, "technical")
        return public_evaluation_payload(cached_answer)

    evaluation_decision = tool_decision(
        "Evaluation Agent",
        "evaluate_round_answer",
        "Evaluate a Technical answer through the structured rubric tool.",
        {
    ...
    if not evaluation_record["ok"]:
        raise HTTPException(status_code=502, detail=evaluation_record.get("error") or "Technical evaluation failed.")
    evaluation = evaluation_record["data"].get("evaluation", {})
    if answer_mode == "code":
        runtime_tool_loop = _empty_runtime_tool_loop("skip_follow_up_for_code_answer")
    elif answer_source in {"pass", "dont_know", "time_expired", "end_call"} or not payload.answer.strip():
    ...
    repository_service.add_round_answer(payload.interview_id, "technical", answer, commit=False)
    runtime_state = advance_runtime_after_answer(interview, "technical", answer)
    repository_service.create_log(
        payload.interview_id,
        "success",
        "Evaluation Agent",
        f"Technical answer scored {answer['score']}/100 with rubric evidence.",
        "technical",
        commit=False,
    )
    await repository_service.commit_local_async()
...
async def get_runtime(interview_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if not interview or interview["userId"] != current_user["id"]:
        raise HTTPException(status_code=404, detail="Interview not found")
    return build_round_runtime_state(interview, "technical")
...
async def start_runtime(interview_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if not interview or interview["userId"] != current_user["id"]:
        raise HTTPException(status_code=404, detail="Interview not found")
    try:
        state = start_round_runtime(interview, "technical", commit=False)
        await repository_service.commit_local_async()
        repository_service.commit_mirror_background()
        return state
    ...
    try:
        response = await llm_service.invoke_live(
            [
                {
                    "role": "system",
                    "content": "Paraphrase technical interview questions neutrally. Return one plain sentence, no markdown.",
                },
                {
                    "role": "user",
        ...
        raise HTTPException(
            status_code=502,
            detail="Live technical paraphrase generation failed. No fallback paraphrase was generated.",
        ) from exc
